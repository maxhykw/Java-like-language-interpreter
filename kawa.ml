...
type binop = Add | Sub | Mul | Div | Rem
           | Lt  | Le  | Gt | Ge | Ass | Eq | Neq
           | Eq_str | Diff_str | And | Or
...
type expr =
  ...
  (* Création d'un tableau avec taille *)
  | ArrayCreate of typ * expr 
  (* Accès à un élément du tableau *) 
  | ArrayAccess of string * expr 
  (* Assignation d'un élément dans le tableau *)
...
type class_def = {
    class_name: string;
    attributes: (string * typ * bool * expr option) list;
