  ...
  let rec exec_seq s lenv =
    ...
    and eval (e: expr): value = 
      ...
      | MethCall (obj_expr, method_name, args) -> (
          ...
          let arg_values = List.map eval args in
          (* Vérifier si l'expression est bien un objet, appeler la méthode *)
    ...
    (* Déplacé en raison de la portée d'une variable *)
    and eval_call f this args =
      match this with
      | VObj { cls; _ } -> (
          (*Cherche la définition de la classe à laquelle l'objet appartient *)
          match List.find_opt (fun c -> c.class_name = cls) p.classes with
          | Some class_def -> (
              (*  Cherche la définition de la méthode dans la classe *)
              match List.find_opt (fun m -> m.method_name = f) class_def.methods with
              | Some mdef -> (
                  let lenv = Hashtbl.create 16 in
                  Hashtbl.add lenv "this" this;
                  (* Initialisation des paramètres (avec les arguments évalués) *)
                  List.iter2
                    (fun (param_name, _) arg_value -> Hashtbl.add lenv param_name arg_value)
                    mdef.params args;
                  (* Initialisation des variables locales définies dans la méthode *)
